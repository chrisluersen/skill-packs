...
1. Pick a design from the catalog below (or pick 2-4 for a **fusion design** — see the Fusion Design section at the end)
2. Load it: `skill_view(name="popular-web-designs", file_path="templates/<site>.md")`
3. Use the design tokens and component specs when generating HTML
4. Pair with the `generative-widgets` skill to serve the result via cloudflared tunnel

### Fusion Design (Using Multiple Templates)

When your artifact needs to combine visual characteristics from multiple design systems — e.g., Supabase's dark background, Linear's thin borders, and Stripe's purple accent — load 2-4 templates and allocate each to a different design dimension:

| Dimension | Example Source |  
|-----------|----------------|
| Background & surfaces | One primary design system |
| Border system | A system with distinctive border language |
| Accent color | A system with a strong brand color |
| Layout pattern | A system with matching information architecture |

See the `html-application-building` skill's **Fusion Design** section for the full pattern, rules, and a worked example (98KB reference doc fusing Supabase + Linear + Mintlify + Stripe).
