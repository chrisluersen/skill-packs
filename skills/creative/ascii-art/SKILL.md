...
### ⚠️ Pitfall: LLM-Generated Animal Art Often Looks Abstract

**Problem:** Freehand LLM ASCII of animals frequently produces unrecognizable blobs (see session where user rejected 3 attempts at dogs).

**Root causes:**
- Over-reliance on block/shade characters (`░░░`) instead of anatomical line work
- Missing breed-defining features: ear set, tail carriage, eye shape/color, marking patterns
- No silhouette-first approach — jumping to texture before structure

**Fix workflow:**
1. **Use reference art first** — Check `ascii.co.uk/art/dog` or `references/dog-breeds.md` for breed templates
2. **Draw silhouette only** — Outline head, ears, body, legs, tail with box-drawing chars
3. **Add defining features** — Eyes (color-coded), ear type, tail type, marking zones (via comments)
4. **Validate against breed standard** — Docked tail? Heterochromia? Merle vs tri vs bicolor?
5. **Only then add texture** — And even then, prefer line work over shade blocks

### Reference Files

- `references/dog-breeds.md` — Australian Shepherd (black tri, docked) & Border Collie (black & white, heterochromia) templates + technique tips
