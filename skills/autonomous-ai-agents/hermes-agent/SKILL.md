...
### Managing Hermes Vault Synchronization

When synchronizing your Hermes vault between local storage and cloud storage (like OneDrive, Google Drive, etc.):

**Naming Convention:**
- Use distinct names for local vs cloud vaults to avoid confusion (e.g., `Hermes-Vault` locally vs `Hermes-Vault-OneDrive` in OneDrive)
- This prevents confusion when both locations are accessible simultaneously

**Consolidation Strategy:**
- Keep related Hermes configuration files together in the cloud vault:
  - `.env` (API keys and secrets)
  - `auth.json` (OAuth tokens)
  - `hermes-dot-env.env` (environment-specific settings)
  - `config.yaml` (Hermes configuration)
  - `backups/` directory for configuration history
- Avoid duplicating large vault contents (like Obsidian notes, media files) unless specifically needed for backup

**Environment Variables:**
When changing your cloud vault location, update relevant environment variables:
```bash
# In .bashrc or .zshrc
export ONEDRIVE_VAULT_ROOT="$HOME/OneDrive/Hermes-Vault-OneDrive"
```

**Backup Management:**
- Regular configuration backups (config.yaml, .env, auth.json) are valuable to keep in cloud storage
- Large vault backups (containing notes, media, etc.) can consume significant space; consider:
  - Keeping only recent versions
  - Using selective sync for large directories
  - Relying on local backup solutions for large vault contents

**Verification:**
After changing vault locations or synchronization setup:
1. Verify Hermes can access required configuration files
2. Check that `.env` and `auth.json` are readable
3. Confirm custom skills and plugins load correctly
4. Test that cron jobs and scheduled tasks still function

#### Profiles As Fleet / Persona Agents

A practical and future-proof pattern for multi-agent setups is to model specialist agents as Hermes profiles rather than separate OS processes. Each profile supplies its own persona prompt, model tier, skills, memory namespace, toolsets, and cron schedule. The user-facing experience then decides whether Hermes stays in front or a specialist profile becomes the default conversation surface.

**Fleet manifest → Hermes profile mapping**

| Manifest field | Profile attribute |
|---|---|
| Designation / nickname | Profile name (lowercase-kebab or short slug) |
| Core role + persona | Persona prompt (in profile context) |
| MBTI, quirks, beverage, welcome phrase | Persona prompt details |
| Model tier + context/output max | Profile `model` override in config |
| Tools / skills | Profile-specific skills |
| Cron schedule + output channel | Cron job targeting that profile |
| Fallback chain | Routing rules (implement in orchestration prompt or dispatcher) |
| Synergistic partner | Shared coordination via wiki `/relates_to` edges |

**Three implementation modes**

| Mode | UX shape | When to use |
|---|---|---|
| **A — Hermes in front** | User always talks to Hermes; fleet profiles delegate underneath | Single consistent surface; lowest friction; safest default |
| **B — Fleet as primary UI** | A supervisor profile becomes the sticky default and Hermes retreats to infra | Distinct supervisor-led experience; user wants the fleet identity visibly in charge |
| **C — Hybrid** | Hermes handles casual chat; fleet activates for complex multi-step work | Best-of-both, but adds switching logic |

Recommend Mode A unless the user explicitly wants the persona switch to be permanent. Changing the default profile is reversible, but it changes the conversational interface surface.
...
[Toad](https://www.batrachian.ai/) is an AI agent manager that discovers ACP-compatible agents via TOML definitions in its `data/agents/` package directory. To make Hermes visible in Toad's agent selector, create a `nousresearch.com.toml` file there.

Full guide with TOML format, pitfalls, install actions, and troubleshooting: `skill_view(name="hermes-agent", file_path="references/toad-agent-registration.md")`.
...
#### Reasoning not showing in responses
If you cannot see the reasoning process in Hermes responses:
1. Check if `display.show_reasoning: true` is set in config.yaml
2. If missing, add:
   ```yaml
   display:
     show_reasoning: true
   ```
3. Restart Hermes: `hermes restart` (or exit and relaunch in CLI)
